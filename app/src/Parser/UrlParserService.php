...
class UrlParserService
{
    ...
    public function __construct()
    {
        $this->client = HttpClient::create(['headers' => [
            ...
            "If-Modified-Since" => "Mon, 25 Oct 2021 13:31:21 GMT"]
        ]);
    }
    ...
    ) : string|null
    {
        if ($customHttpClient !== null) {
            ...
            $response = $this->client->request($requestMethod, $url);
        }
        try {
     ...
     */
    public function makeCustomHttpClient(array $headers)
    {
     ...
     * @return Cookie
     */
    public function createCookie(array $cookie): Cookie
    {
        return new Cookie(
            $cookie['name'],
            $cookie['value'],
     ...
     * @return Client
     */
    public function summonPantherClientAndMakeRequest
    (
        string $url,
        string $pathToDriver,
        array  $cookieInfo = [],
    ...
    /**
     * @param array $targetSubDoms
     * @param string $url
     * @return array
     */
    public function prepareUrlsWithSubDomain(array $targetSubDoms, string $url): array
    {
        $result = [];
        $url = parse_url($url);
     ...
     * @return array
     */
    public function prepareUrlsWithHrefs(array $hrefs, string $url): array
    {
        $result = [];
        $url = parse_url($url);
